...
[source,java]
.Create a new class GreetHealthcheck with the following content
----
...
public class GreetHealthCheck implements HealthCheck {
  ...
  public HealthCheckResponse call() {
    ...
    return HealthCheckResponse.named("LivenessCheck") // <2>
        .up()
...
----
<1> Indicates that this is a liveness health check
<2> Build the HealthCheckResponse with status UP and the current time
