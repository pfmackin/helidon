...
using the CDI.  The following example shows how to add a custom liveness health check

[source,java]
.Create a new class GreetLivenessCheck with the following content
----
...
----
<1> Annotion specifies that this is a liveness health check
<2> Annotion specifies that there is a single LivenessCheck object during the lifetime of the application
<3> Build the HealthCheckResponse with status UP and the current time

[source,bash]
.Build and restart the application, then verify the custom liveness health endpoint
----
...
[source,java]
.Create a new class GreetReadinessCheck with the following content
----
...
<2> Annotion specifies that this is a readiness health check
<3> Build the HealthCheckResponse with status UP after 5 seconds else DOWN
...
[source,bash]
.Get all the health check data, including custom, single query
----
curl http://localhost:8080/health
----
...
[source,bash]
.Verify the health endpoints using port '30116'.  NOTE: your port may be different.  For example
----
