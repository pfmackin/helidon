...
|Use the <<guides/02_quickstart-se.adoc,Quickstart SE>> guide to create a project
|===
...
[source,java]
.Modify Main.java and replace the createRouting method with the following:
----
...
[source,java]
.Add a readyTime variable in the Main, then set it 5 seconds after the server starts.
----
import java.util.concurrent.atomic.AtomicLong;public final class Main {

  private static AtomicLong readyTime = new AtomicLong(0); // <1>
  ...
    ...
    ...
      ...
      return server;
----
<1> Declare the ready time variable
<1> Sleep 5 seconds
<1> Set the readyTime to the time when the server was ready
