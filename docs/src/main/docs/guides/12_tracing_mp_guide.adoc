...
A _trace_ contains a collection of spans, where spans are organized as a directed acyclic graph (DAG) and can belong to multiple services,
...
but its descendants can belong to different services and hosts.  OpenTracing uses a `SpanContext` to
