...
For example, if you specify a custom server port in `application.yaml`
...
3. Properties specified in `application.yaml`
...
[source,bash]
.Change `app.greeting` in the application.yaml` from `Hello` to `HelloFromMPConfig`:
----
app:
  greeting: HelloFromMPConfig
----
...
[source,java]
.Update the `Main` class; 1) Add new imports and 2) Replace the `startServer` method:
----
...
[source,yaml]
.Create a file `config-file.yaml` in the `helidon-quickstart-se` directory and add the following contents:
----
app:
  greeting:
    sender: Joe
