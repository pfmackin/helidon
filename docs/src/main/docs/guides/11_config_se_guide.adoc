...
[source,bash]
.Change `app.greeting` in `application.yaml` as follows:
----
app:
...
[source,java]
.Update the `Main` class; 1) Add new imports, 2) Replace the `Config.create()` call with `buildConfig()` , 3) Add `buildConfig` method:
----
...
[source,java]
.Replace `GreetService` constructor with the following code:
----
    GreetService(Config config) {
...
The Helidon `Config` class provides several methods that allow you to filter and tailor the traversing of
configuration trees.  The example below shows how to get the `greeting` node when you only know it is somewhere in
the `app` subtree.
...
<3> Remove any node that does not have the name `greeting`.
...
allows it to watch sources for changes. Your application listen on updates to the underlying config sources and react to the changes.
See <<config/05_mutability-support, Config Mutability Support>> for a full discussion on this topic.
The following example demonstates how to listen and react to configuraiton changes.
...
[source,bash]
.After a few seconds and check the response:
----
curl http://localhost:8080/greet
...
...
----
<1> The application reacted to the change and updated the in memory greeting.
...
data:
  config-file.properties: |   // <1>
    app.greeting=HelloFrom-config-file.properties   // <2>
kind: ConfigMap
...
----
