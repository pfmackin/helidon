...
When using multiple annoations on a method you *must* give the metrics different names using the annoation `name` field
...
----
<1>  Both /birthday and /wedding use the same Counter metric, named `specialEventCard`
...
The metrics we have examined so far are updated in response to an applicaiton REST request, i.e GET /cards.  These
...
<2> Annotation will allow Prometheus to discover and scrape this pod.
...
Prometheus to discover the service and scrape the metrics.  Lets install Prometheus and see that it discovered the Helidon metrics;
...
Helidon applicaiton as one of the Prometheus targets.


[source,bash]
.Delete the prometheus resources
----
