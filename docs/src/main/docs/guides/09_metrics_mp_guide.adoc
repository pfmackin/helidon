...
one web service invocation and it is destroyed at the end of that
invocation.
<3> The annotation @Counted will result in Helidon creating and registering a counter metric in the MetricRegistry.
The counter is incremened each time the anyCard() method is called.  The `name` and `monotonic' fields are optional.
...
----
<1> The an-card count will be 2, since you invoked the endpoint twice.
...
----
<1> Specify a custom name for the Counter metric and also set `absolute=true` to remove the path prefix from the name
...
The following example introduces a metric to count all card queries.  In this example, the method level are not needed,
but they are left in the example to demonstrate the combined output of all 3 metrics.
...
<4> Return the application up-time which will be included in the application metrics
