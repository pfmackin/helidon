...
You can include configuation data from a variety of sources using different formats, like JSON and YAML.
Furthermore, you can customize the precedence of sources and make them optional or mandatory.
This guide introduces Helidon MP configuration and demonstates the fundamental concepts using several examples.
Refer to <<config/config/01_introduction, Helidon Configuration>> for the full configuration concepts documentation.
...
An environment variable has a higher precedence thay the configuration properties file.
...
[source,json]
.Verify that the output matches the string below:
----
{
  "message": "HelloFromConfigFile World!"
}
...
<4> Specify the ConfigMap that contains the configuration data.
