...
<5> You must specify the existing `META-INF/microprofile-config.properties` or Helidon will not use it as a Config source
even though it is considered a default source.

[source,bash]
.Build and run the application (without the system property), then invoke the endpoint and check the response:
----
curl http://localhost:8080/greet
...
{
...
[source,java]
.Update the `Main` class; 1) add new import and 2)replace the `startServer` method:
----
...
sources:
  ...
  - type: "classpath"  // <3>
    properties:
      resource: "META-INF/microprofile-config.properties" // <4>
  - type: "file"
    properties:
...
<3> The source type specifies a classpath resource.
<4> The name of the mandatory resource.
<5> Specify that the `optional-config-file` file is optional.
...
a source if you want them used.
...
[source,java]
.Update the `GreetingProvider.java` file; 1) add new import and 2)replace the `GreetingProvider` class:
----
...
public class GreetingProvider {
    ...
    public void onStartUp(@Observes @Initialized(ApplicationScoped.class) Object init) {
        message.set(config.get("app.greeting").asString().get()); // <2>
    }
...
...
----
<1> The file `config-file.properties` will be created within the Kubernetes container in `/etc/config`.
