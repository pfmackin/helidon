...
sources:
  ...
  - type: "file"  // <3>
    properties:
      path: "config-file.properties" // <4>
----
<1> The source type.
<2> The name of the mandatory configuration resource.
<3> The second source type
<3> The path of the mandatory configuration file.
...
You can re-run the previous tests that exercised environment variables and system properties.
